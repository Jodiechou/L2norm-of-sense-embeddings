...
def get_args(
	...
	parser.add_argument('-sv_path', help='Path to sense vectors', required=False, default='data/lmms_2048.bert-large-cased.fasttext-commoncrawl.npz')
	# parser.add_argument('-sv_path', help='Path to sense vectors', required=False, default='external/ares/ares_bert_large.txt')
	parser.add_argument('-clf_orig_path', help='Path to .pkl orginal LR classifier', required=False, default='data/models/wic_binary_lmms2048_glove.pkl')
	parser.add_argument('-clf_l2norm_path', help='Path to .pkl l2norm LR classifier', required=False, default='data/models/wic_binary_lmms_glove_set0.pkl')
...
class SensesVSM(object):
	...
	def get_senseEmbeddings(self, label):
		if label not in self.sense_embed.keys():
			sense_vec = None
		else:
			sense_vec = self.sense_embed[label]
		return sense_vec	## For npz file
...
if __name__ == '__main__':
	...
	clf_orig = joblib.load(args.clf_orig_path)
	# clf_l2norm = joblib.load(args.clf_l2norm_path)

	results_path = 'data/results/wic.classify.%s.lmms.glove.txt' % args.eval_set
	# results_path = 'data/results/wic.classify.%s.ares.original.txt' % args.eval_set
	...
	with open(results_path, 'w') as results_f:  # store results in WiC's format
		for wic_idx, wic_entry in enumerate(load_wic(args.eval_set, wic_path='external/wic')):
			...
			# 	pred = clf_orig.predict([[s1_sim, s2_sim, s3_sim, s4_sim]])[0]
			# else:
			...
			# 	pred = clf_l2norm.predict([[s1_sim, s2_sim, s3_sim, s4_sim, ex1_l2_sense_vec, ex2_l2_sense_vec]])[0]
			'''********************'''

			'''Uncomment thses lines for binary classifier with l2 norm GloVe-sense or SGNS-sense and set l2norm = 0 for missing senses'''
			# if ex1_matches[0][0] not in glove.dictionary or ex2_matches[0][0] not in glove.dictionary:
			# # if ex1_matches[0][0] not in word2vec.wv.vocab or ex2_matches[0][0] not in word2vec.wv.vocab:
			# 	pred = clf_l2norm.predict([[s1_sim, s2_sim, s3_sim, s4_sim, 0, 0]])[0]
			# else:
			# 	ex1_l2_sense_vec =np.linalg.norm(glove.word_vectors[glove.dictionary[ex1_matches[0][0]]], ord=2)
			# 	ex2_l2_sense_vec =np.linalg.norm(glove.word_vectors[glove.dictionary[ex2_matches[0][0]]], ord=2)
			# 	# ex1_l2_sense_vec =np.linalg.norm(word2vec[ex1_matches[0][0]], ord=2)
			...
			'''Uncomment out this line for original binary classifier'''
			pred = clf_orig.predict([[s1_sim, s2_sim, s3_sim, s4_sim]])[0]
			'''********************'''
